<?php 
	...
	$mens = mysql_fetch_array(mysql_query("select *, nome_categoria from preco_mensalidade join categorias on categoria_fk = id_categoria where id_preco_mensalidade=$id;",$db));
	//Selecionar dados da mensalidade escolhida
...
<div>
    <ul class="breadcrumb">
		...
		<li>
            <a href="editar_mens.php">Editar Preço Mensalidade</a>
        </li>
    </ul>
</div>
<!-- Fim breadcrumb -->
    ...
    <div class="box col-md-4">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i> Editar Preço Mensalidade</h2>
            </div>
					...
					<form action="cadastrar_preco_mensalidade.php" method="POST" name="cad_preco_mens">
						<div class="form-group" >	
							...
							<select name="horario" data-rel="chosen" required > 
								<option value="<?php echo $mens['horario']?>"><?php echo utf8_encode($mens['horario'])?>º Turno</option>
						...
						<div class="form-group" >
							<label>Valor:(R$)</label>
							<input type="text" name="valor" id="valor"  class="form-control" placeholder="Informe o valor no formato 00.00" onblur='casas(this.value,this)' onkeyup='calcular()' value="<?php echo $mens['valor']?>" />
						</div>
						<div class="form-group" >	
							<label>Desconto:(%)</label>
							<input type="text" name="desconto" id="desconto" value="<?php echo $mens['desconto']?>" class="form-control" placeholder="informe a porcentagem no formato 00.00" required onblur='casas(this.value,this)' onkeyup='calcular()' />
						</div>
						<div class="form-group" >					
						...
						<input type="hidden" name="id" value="<?php echo $mens['id_preco_mensalidade']?>" />
						
						<input type="hidden" name="acao" value="editar" />
											
						<br/>
						<button type="submit" class="btn btn-success">Alterar</button>
						<a href="editar_mens.php"><button type="button" class="btn btn-info">Limpar</button></a>
