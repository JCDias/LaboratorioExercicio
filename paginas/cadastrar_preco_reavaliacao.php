<?php 
	...
	if($acao=="cadastrar"){
		...
		$sql = "INSERT INTO `preco_reavaliacao`(`valor`, `categoria_fk`, `funcionario`, `data_cadastro`) VALUES ('$valor',$categoria,'$funcionario',now());";
		//Fim Preparar consulta para inserir
	}else{
		//Preparar consulta para Atualizar
		$id = $_POST['id'];
		$sql = "UPDATE `preco_reavaliacao` SET `valor`='$valor',`categoria_fk`=$categoria,`funcionario`='$funcionario',`data_cadastro`=now() WHERE `id_preco_reavaliacao` = $id";
